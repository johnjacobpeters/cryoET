...
%
%  %Read SuperRes and convert to .em for GPU (dimmy) frame Alg
%  %In DM File-->Save as-->data only 
%  i=tom_rawread('myGain.raw','float','le',[7676 7420 1],0,0);    
%  tom_emwrite('myGain.em',i);
%  
%  
% 
%
...
if (strcmp(form,'uint8'))
    Data_read=zeros(xdim,ydim,zdim,form);
else
    ...
    elseif isequal('int8',form)
    	for lauf=1:zdim
    		Data_read(:,:,lauf) = fread(fid,[xdim,ydim],'int8');
    	end;
