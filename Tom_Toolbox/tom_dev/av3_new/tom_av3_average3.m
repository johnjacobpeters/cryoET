function averageVol=tom_av3_average3(Align,method,threshold,iclass,alg_part_path,waitbarflag)
...
icount = 0;
waitbarCL=tom_progress(size(Align,2),'averaging particles');
for indpart = 1:size(Align,2)
    if (Align(indpart).CCC)>=threshold &&  (isempty(find(ismember(iclass,Align(indpart).Class )))==0 || iclass==-1)  
        icount = icount +1;
        ...
        if (tom_isemfile(name)==1)
            particle = tom_emreadc(name); 
        else
            particle = tom_mrcread(name); 
        end;
        ...
        averageVol = averageVol + particle;
...
waitbarCL.close();
lowp = floor(size(averageVol,1)/2)-3;
wei = 1./wei;
rind = find(wei > 100000);
wei(rind) = 0;% take care for inf
averageVol = real(tom_ifourier(ifftshift(tom_spheremask(fftshift(tom_fourier(averageVol)).*wei,lowp))));
